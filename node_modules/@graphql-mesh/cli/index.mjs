...
import { defaultImportFn, loadYaml, DefaultLogger, writeFile, pathExists, writeJSON, loadFromModuleExportExpression, rmdirs } from '@graphql-mesh/utils';
import Ajv from 'ajv';
...
import { getRootTypeMap, buildOperationNodeForField, parseGraphQLSDL, printSchemaWithDirectives } from '@graphql-tools/utils';
...
function validateConfig(config, filepath, initialLoggerPrefix) {
    ...
    if (!isValid) {
        ...
        logger.warn(`${filepath} configuration file is not valid:\n${ajv.errorsText(ajv.errors, {
            separator: '\n',
...
async function serveMesh({ baseDir, argsPort, getBuiltMesh, logger, rawServeConfig = {}, playgroundTitle }, cliParams) {
    const { fork, port: configPort, hostname = platform() === 'win32' ||
        ...
        ? '127.0.0.1'
        : '0.0.0.0', cors: corsConfig, handlers, staticFiles, playground: playgroundEnabled = process.env.NODE_ENV !== 'production', sslCredentials, endpoint: graphqlPath = '/graphql', browser, trustProxy = 'loopback', } = rawServeConfig;
...
async function graphqlMesh(cliParams = DEFAULT_CLI_PARAMS, args = hideBin(process.argv), cwdPath = process.cwd()) {
    ...
    return yargs(args)
        ...
        try {
            ...
            meshInstance$
                .then(({ schema }) => writeFile(path.join(outputDir, 'schema.graphql'), printSchemaWithDirectives(schema)))
                .catch(e => {
