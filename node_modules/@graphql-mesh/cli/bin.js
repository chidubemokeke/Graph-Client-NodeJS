...
async function findAndParseConfig(options) {
    ...
    return config.processConfig(config$1, { dir, initialLoggerPrefix, ...restOptions });
}
...
async function serveMesh({ baseDir, argsPort, getBuiltMesh, logger, rawServeConfig = {}, playgroundTitle }, cliParams) {
    const { fork, port: configPort, hostname = os.platform() === 'win32' ||
        ...
        ? '127.0.0.1'
        : '0.0.0.0', cors: corsConfig, handlers, staticFiles, playground: playgroundEnabled = crossHelpers.process.env.NODE_ENV !== 'production', sslCredentials, endpoint: graphqlPath = '/graphql', browser, trustProxy = 'loopback', } = rawServeConfig;
    const port = argsPort || parseInt(crossHelpers.process.env.PORT) || configPort || 4000;
        ...
        await Promise.all((handlers === null || handlers === void 0 ? void 0 : handlers.map(async (handlerConfig) => {
            ...
            if ('handler' in handlerConfig) {
                ...
                handlerFn = (req, res) => {
                    ...
                    const pubsubTopic = stringInterpolation.stringInterpolator.parse(handlerConfig.pubsubTopic, interpolationData);
