...
const createMetaSelectionNode = utils.memoize1(function createMetaSelectionNode(config) {
    return {
...
const validateSchema = utils.memoize2(function validateSchema(schema, config) {
    const metaType = schema.getType(config.metaTypeName);
    if (metaType == null || !graphql.isObjectType(metaType)) {
    ...
    const blockType = graphql.getNamedType(blockField.type);
    if (!graphql.isObjectType(blockType)) {
    ...
    const metaQueryFieldType = graphql.getNamedType(metaQueryField.type);
    if (!graphql.isObjectType(metaQueryFieldType) || metaQueryFieldType.name !== config.metaTypeName) {
    ...
    for (const fieldName in queryFields) {
        ...
        const blockArgumentType = graphql.getNamedType(blockArgument.type);
        if (!graphql.isInputObjectType(blockArgumentType)) {
...
const getQueryFieldNames = utils.memoize1(function getQueryFields(schema) {
    const queryType = schema.getQueryType();
    if (queryType == null) {
        throw new Error(`Make sure you have a query type in this source before applying Block Tracking`);
    }
...
class BlockTrackingTransform {
    ...
    transformRequest(executionRequest, delegationContext) {
        ...
        const document = graphql.visit(executionRequest.document, {
            Field: {
            ...
            OperationDefinition: {
                leave: (operationNode) => {
                    ...
                    if (operationNode.operation === graphql.OperationTypeNode.QUERY && shouldAddMetaField) {
...
module.exports = BlockTrackingTransform;
