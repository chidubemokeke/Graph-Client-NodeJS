...
const validateSchema = utils.memoize2(function validateSchema(schema, config) {
    ...
    for (const fieldName in queryFields) {
        ...
        const nullableType = graphql.isNonNullType(field.type) ? field.type.ofType : field.type;
...
class AutoPaginationTransform {
    ...
    transformSchema(schema, subschemaConfig, transformedSchema) {
                            ...
                            if (totalRecords >= this.config.skipArgumentLimit * 2) {
                                ...
                                while (remainingRecords > 0) {
                                    ...
                                    while (skipValue < this.config.skipArgumentLimit && remainingRecords > 0) {
                                        ...
                                        const result = await delegate.delegateToSchema({
                    ...
                    for (const selectionNode of selectionSet.selections) {
                        if (selectionNode.kind === graphql.Kind.FIELD &&
                            ...
                            if (firstArg != null) {
                                ...
                                if (firstArg.value.kind === graphql.Kind.INT) {
                                    numberOfTotalRecords = parseInt(firstArg.value.value);
                                }
                                else if (firstArg.value.kind === graphql.Kind.VARIABLE) {
                                ...
                                if (numberOfTotalRecords != null && numberOfTotalRecords > this.config.limitOfRecords) {
                                    ...
                                    let initialSkip = 0;
                                    if (((_e = skipArg === null || skipArg === void 0 ? void 0 : skipArg.value) === null || _e === void 0 ? void 0 : _e.kind) === graphql.Kind.INT) {
                                        initialSkip = parseInt(skipArg.value.value);
                                    }
                                    else if (((_f = skipArg === null || skipArg === void 0 ? void 0 : skipArg.value) === null || _f === void 0 ? void 0 : _f.kind) === graphql.Kind.VARIABLE) {
...
module.exports = AutoPaginationTransform;
